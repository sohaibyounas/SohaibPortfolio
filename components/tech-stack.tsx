  ...
  return (
    <div className="relative">
      <motion.button
        ...
        className="relative w-full rounded-lg border border-border bg-muted/50 px-3 py-2.5 text-left font-mono text-xs text-foreground transition-all hover:border-foreground/20 hover:text-foreground"
        whileHover={{ y: -2 }}
      ...
      <motion.div
        ...
        className="pointer-events-none absolute -top-10 left-1/2 z-20 -translate-x-1/2 max-w-[220px] text-center rounded-md border border-border bg-card px-2.5 py-1.5 text-xs text-foreground shadow-lg hidden sm:block"
      >
        {desc}
        ...
        <div className="grid gap-8 sm:gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {CATEGORIES.map((cat, i) => (
            <SectionReveal key={cat.title} delay={i * 0.08}>
              <div>
                <h3 className="mb-4 font-mono text-xs font-semibold tracking-[0.15em] text-foreground uppercase">
                  {cat.title}
                </h3>
